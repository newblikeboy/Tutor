...
type Config struct{ Env, Name, Addr, Origin, URI, Database, OTPSecret, AuthProvider string }
...
func Load() (Config, error) {
	c := Config{env("APP_ENV", "development"), env("APP_NAME", "Tutor Platform"), env("HTTP_ADDR", "127.0.0.1:8080"), env("WEB_ORIGIN", "http://127.0.0.1:5173"), os.Getenv("MONGODB_URI"), env("MONGODB_DATABASE", "tutor_dev"), os.Getenv("OTP_SECRET"), env("AUTH_PROVIDER", "development")}
	if c.URI == "" {
		return c, errors.New("MONGODB_URI is required; no database fallback")
	}
	if len(c.OTPSecret) < 32 {
		return c, errors.New("OTP_SECRET must contain at least 32 random characters")
	}
	...
	if c.AuthProvider != "development" && c.AuthProvider != "disabled" {
