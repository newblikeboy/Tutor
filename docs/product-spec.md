...
> User Updates override (2026-09-26, latest): remove end-to-end encryption. Updates is a simple one-way inbox: admins send to parents/tutors, approved tutors send to assigned parents, and senders see recipient-only read receipts. No replies, inbox activation, passphrase or device linking. Keep HTTPS, session/CSRF and assignment controls. New messages are stored server-side; old encrypted records are retained separately without automatic conversion. See `updates-inbox.md`.
