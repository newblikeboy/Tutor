...
> User Updates override (2026-09-26): Updates is a one-way end-to-end encrypted inbox: admins send to parents/tutors, approved tutors send to their assigned parents, recipients cannot reply, and senders see signed recipient read receipts. Private keys/passphrases stay client-side; preserve assignment revocation and separate existing Activity/tuition-chat behaviour. See `encrypted-inbox.md` for the implemented key lifecycle and security limits.
> User password-policy override (2026-09-25): lower the minimum from 15 to 8 characters across signup, password changes and staff provisioning. Keep the 128-character maximum and existing hashing/session/access protections; existing passwords stay valid.
> User staff-auth override (2026-09-25): the operator explicitly selected email/password-only login for provisioned staff, overriding the earlier mandatory staff MFA requirement. Public signup remains parent/tutor only; preserve least-privilege role/assignment checks, rate limits, Secure sessions, CSRF and sample-account rejection. Guardian and payment boundaries remain unchanged.
