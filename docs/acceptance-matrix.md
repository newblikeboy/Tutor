...
| §3 Editorial home, standards, continuity/FAQ | `/`, `/standards`, `/approach`, `/how-it-works`; photo hero, balanced grids, account palette | E2 + V; `home.spec.ts` checks real discovery/requirement entry links, keyboard FAQ and loading/failure/retry | Homepage refined with labelled generated imagery; preserves actual API-backed listings. Operator content review pending |
...
| §3 Parent/tutor/mentor/admin attention; user dashboard redesign | Separate private shell on `/workspace`, `/match`, `/apply`; sidebar/mobile drawer, dedicated role views, real count summaries | E1, V; four role cases in `workspace.spec.ts`; two reviewed Windows baselines in `workspace-visual.spec.ts` | Working bounded dashboards with the approved colour palette; broader operational modules remain pending |
| User dashboard learner/navigation persistence | Owned learner selection, selected view and staff search/status in URL state | `workspace.spec.ts`: actual signup + two persisted learners/requirements, selection across views/reload; assigned queue search/no-match/reset/reload | Real API records only; API queries remain capped at 100 and cursor pagination is pending |
| User dashboard access/error states | Session gate, loading, request failure/retry, actual logout, role denial | `workspace.spec.ts` failure injection/recovery and provisioned support-role denial | Checked implemented routes; existing Go ownership and assignment checks remain authoritative |
| §4 Responsive widths 360/390/768/1024/1440 | Homepage grid + core mobile layouts | E2 and `home.spec.ts`: both languages at all five widths, hero alignment, decoded image, no overflow; V | Verified listed viewport cases; not every future route/state |
...
| §8 Queues/audit/status | Mentor decisions and admin audit/suspension; name/status filters and separate task views | Go-I audit effects, E1, V, `workspace.spec.ts` persisted filters | Working bounded queues and current-record counts; server pagination, capacity/financial/CMS/operations reports pending |
