...
The bootstrap host's ability to read database credentials is privileged; restrict it and independently review the recorded operator reference. Production still requires `AUTH_PROVIDER=disabled`: provisioning alone does not bypass the release gate. Staff MFA enrollment/recovery, staff permission-change UI, email ownership verification and actual invitation delivery remain separate implementation/release requirements. No production staff account has been created by this work.
