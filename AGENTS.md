...
- Windows loads private `.env` with `./scripts/api.ps1 api|migrate|seed`. Run Atlas integration with `./scripts/integration.ps1 -Race`. E2E starts isolated ports/databases; never point fixtures at production. Six screenshot baselines are Windows-specific and were visually reviewed; do not auto-update them.
